...
for(const required of ['Accounting core 4.4.2','expInsuranceContribution','Insurance payment route','Still owed','Invoice totals, credit notes, payments and outstanding balances are separate','Only expenses dated in the selected quarter','A.receivables(data,p)','A.controls(data,new Date(),p)'])if(!bookkeepingUi.includes(required))throw new Error(`Bookkeeping v4.4.2 workflow is missing ${required}`);
...
for(const required of ['openCorrectionPeriods','prepareV44PaidDateRepair','affected accounting period is closed',"AccountingSchema='4.4.2'"])if(!bookkeepingUi.includes(required))throw new Error(`Paid-invoice correction workflow is missing ${required}`);
...
if(!fs.readFileSync(path.join(root,'service-worker.js'),'utf8').includes('renoweet-focus-hub-v4.4.2-19'))throw new Error('Service-worker cache version was not bumped for v4.4.2.');
