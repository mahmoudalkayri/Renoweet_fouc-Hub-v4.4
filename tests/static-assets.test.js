...
for(const required of ['renderAutoFuel','expenseSources(p)',"mainCategory==='Fuel'","mainCategory==='Automobile'","autoFuel:'Auto & Fuel'","targetArr=originalType==='Fuel'?data.fuel:originalType==='Automobile'?data.auto:data.expenses","'Main category':originalType"])if(!bookkeepingUi.includes(required))throw new Error(`Quarter-aware Auto & Fuel workflow is missing ${required}`);
...
for(const required of ['renoweet-os-vat-engine-v1.js','class="e_vat"','VAT Reference Rate','vatBreakdownRows',"t.mixed?'MIXED'",'Standaard-BTW voor nieuwe regels / reiskosten'])if(!(osPage+bookkeeping+bookkeepingDrive).includes(required))throw new Error(`OS mixed-rate quotation workflow is missing ${required}`);
...
for(const required of ['VATAccountingBasis','Reverse-charge reference rate','Received reverse-charge VAT','VehiclePrivateUseVatByYear','PeriodAdjustments','Net cash movement — recorded transactions','Supplier cash paid'])if(!(bookkeepingUi+fs.readFileSync(path.join(root,'renoweet-accounting-engine-v4.4.js'),'utf8')).includes(required))throw new Error(`Accounting improvement is missing ${required}`);
for(const required of ['annualTaxPlanning','YearTaxSettings','Annual income-tax planning','Recommended tax reserve','Still to reserve','saveV44YearTaxSettings'])if(!(bookkeepingUi+fs.readFileSync(path.join(root,'renoweet-accounting-engine-v4.4.js'),'utf8')).includes(required))throw new Error(`Annual income-tax planning is missing ${required}`);
for(const required of ['annualIncomeTaxPack','fixedAssetReport','financeLeaseReport','Owner Transactions','AnnualClosing','VehicleUseByYear','Annual Income Tax Pack'])if(!(bookkeepingUi+bookkeeping+driveCore+fs.readFileSync(path.join(root,'renoweet-accounting-engine-v4.4.js'),'utf8')).includes(required))throw new Error(`Annual income-tax pack is missing ${required}`);
if(!fs.readFileSync(path.join(root,'service-worker.js'),'utf8').includes('renoweet-focus-hub-v4.4.4-33'))throw new Error('Service-worker cache version was not bumped for mixed-rate quotations.');
