...
for(const required of ['openCorrectionPeriods','prepareV44PaidDateRepair','affected accounting period is closed',"AccountingSchema='4.4.3'"])if(!bookkeepingUi.includes(required))throw new Error(`Paid-invoice correction workflow is missing ${required}`);
...
if(!fs.readFileSync(path.join(root,'service-worker.js'),'utf8').includes('renoweet-focus-hub-v4.4.3-20'))throw new Error('Service-worker cache version was not bumped for v4.4.3.');
