...
assert.equal(m.rev,9648.78,'issued revenue must equal Q3 net sales ex VAT');
assert.equal(m.paidInvoiceNet,9648.78,'fully settled invoices are a separate net measure');
assert.equal(m.cash,A.cashReport(book,q3).customerCashIn,'customer cash must come from dated payment records');
assert.deepEqual(Array.from(m.monthKeys),['2026-07','2026-08','2026-09'],'chart must include empty calendar months');
...
context.D.bookkeepingPayments=payments.map(p=>({...p,date:'2026-10-15'}));
vm.runInContext('result=metricsFor({year:2026,mode:"Q3"});',context);
assert.equal(context.result.rev,9648.78,'issued revenue stays in the invoice quarter');
assert.equal(context.result.cash,0,'cash moves to the payment quarter');
