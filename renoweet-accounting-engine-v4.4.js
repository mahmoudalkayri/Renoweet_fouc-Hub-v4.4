...
(function(root,factory){
  ...
  function vatReport(book,period){
    ...
    const issuedSalesNet=round2(sales.net),issuedOutputVat=round2(sales.vat),issuedSalesGross=round2(sales.gross),creditNet=round2(credits.net),creditVat=round2(credits.vat),creditGross=round2(credits.gross),salesNet=round2(issuedSalesNet-creditNet),salesGross=round2(issuedSalesGross-creditGross),outputVat=round2(issuedOutputVat-creditVat),inputVat=round2(purchases.inputVat),position=round2(outputVat-inputVat);
    return {issuedSalesNet,issuedOutputVat,issuedSalesGross,creditNet,creditVat,creditGross,salesNet,salesGross,outputVat,inputVat,position,issued,expenses:rows.expenses,creditNotes:rows.creditNotes};
  }
  ...
  function controls(book,today=new Date(),period){
    ...
    for(const r of rows.invoices){const no=text(r['Invoice #']),st=invoiceState(r,book.payments,book.creditNotes,today),t=invoiceTotals(r),rec=reconciliation.get(invoiceId(r));if(no){seen.set(no,(seen.get(no)||0)+1)}if(isIssued(r)&&!no)issues.push({kind:'sales',level:'error',message:'Issued invoice without a number',id:invoiceId(r)});if(isIssued(r)&&!text(r.Customer))issues.push({kind:'sales',level:'error',message:`${no||'Invoice'} is missing a customer`,id:invoiceId(r)});if(isIssued(r)&&!text(r.Address))issues.push({kind:'sales',level:'warning',message:`${no||'Invoice'} is missing a customer address`,id:invoiceId(r)});if(rec?.status==='missing_paid_date')issues.push({kind:'payments',level:'error',message:`${no||'Invoice'} is marked Paid but has no paid date`,id:invoiceId(r)});else if(rec?.status==='missing_payment')issues.push({kind:'payments',level:'error',message:`${no||'Invoice'} is marked Paid but has no payment ledger entry`,id:invoiceId(r)});else if(rec?.status==='mismatch')issues.push({kind:'payments',level:'error',message:`${no||'Invoice'} is marked Paid, but ledger payments are ${round2(rec.paid).toFixed(2)} and the balance due is ${round2(rec.due).toFixed(2)}`,id:invoiceId(r)});if(st.status==='overdue'&&!rec?.isMarkedPaid)issues.push({kind:'sales',level:'warning',message:`${no||'Invoice'} is overdue (${round2(st.outstanding).toFixed(2)} outstanding)`,id:invoiceId(r)});if(st.status==='partially_paid'&&!rec?.isMarkedPaid)issues.push({kind:'payments',level:'info',message:`${no||'Invoice'} is partially paid`,id:invoiceId(r)});if(st.overpaid>.009)issues.push({kind:'payments',level:'error',message:`${no||'Invoice'} is overpaid by ${round2(st.overpaid).toFixed(2)}`,id:invoiceId(r)});if(Math.abs(t.net+t.vat-t.gross)>.009)issues.push({kind:'vat',level:'error',message:`${no||'Invoice'} totals do not balance`,id:invoiceId(r)})}
