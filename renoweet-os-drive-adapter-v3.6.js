...
async function connectDrive(interactive=true){
 try{
  ...
  const identity=await RenoweetDrive.inspectDatabaseFiles(state.year),ignored=identity.outside.length?` • ${identity.outside.length} same-name file(s) outside live year folders ignored`:'';
...
async function installRecoveryDatabase(file){
 if(!file)return;if(file.size>10*1024*1024){alert('Choose the verified Renoweet-2026.json recovery file.');return}
 try{
  const candidate=await RenoweetDrive.verifyCanonical(JSON.parse(await file.text())),info=candidate.recoveryMerge;
  if(!info?.baseChecksum)throw new Error('This is not a verified Renoweet recovery merge file.');
  if(state.readOnly)throw new Error(`Historical year ${state.year} is read-only.`);
  const counts=`${candidate.os.projects.length} active projects, ${candidate.os.deleted.length} deleted-project records, ${candidate.bookkeeping.invoices.length} invoices and ${candidate.bookkeeping.expenses.length} expenses`;
  if(!confirm(`Install this verified recovery merge into the authoritative Renoweet-${candidate.meta.year}.json file?\n\n${counts}\nRevision ${candidate.meta.revision}\n\nRenoweet will first create a Drive Recovery snapshot and will stop if the live database changed after the merge was prepared.`))return;
  if(!state.connected&&!(await connectDrive(true)))return;
  status('Installing verified recovery…','Creating a safety snapshot and checking the exact source checksum');
  const installed=await RenoweetDrive.installRecoveryMerge(candidate);applyRemote(installed.data.os,false);state.year=installed.data.meta.year;state.sectionRevision=installed.data.meta.sections.osRevision;state.baseline=RenoweetDrive.clone(installed.data.os);window.__renoweetCanonical=RenoweetDrive.clone(installed.data);clearRetry();await cacheBrowserState(false);render();status('Recovery installed + verified ✓',`Renoweet-${state.year}.json • revision ${installed.data.meta.revision} • checksum verified`);alert('The merged database is installed and verified. The previous live version is retained in the Drive Recovery folder.')
 }catch(e){console.error(e);status('Recovery installation stopped',e.message);alert(e.message)}
}
async function organizeDriveYear(){
 try{
  if(!state.connected)await connectDrive(true);
  if(!state.connected)return;
  if(state.readOnly){alert(`Switch to ${state.activeYear} before organizing the live year.`);return}
  const p=await RenoweetDrive.planYearOrganization(state.year);
  if(p.alreadyOrganized){alert(`${state.year} is already in its year folder. No files need moving.`);return}
  const detail=`Live JSON: ${p.needsLiveMove?'move the same verified file ID':'already in its year folder'}\nReceipts: ${p.proofCount} in ${p.quarters.map(q=>q.quarter).join(', ')||'no old quarter folders'}\n\nA checksum-verified Recovery copy will be made before any move. Existing receipts keep their Drive IDs; old folders are left for manual review.`;
  if(!confirm(`Organize Renoweet ${state.year} into the year folder?\n\n${detail}`))return;
  status('Organizing year…','Making a verified Recovery snapshot and preserving file IDs');
  const result=await RenoweetDrive.organizeYear(state.year);
  const live=await RenoweetDrive.loadExistingYear(state.year);
  if(!live||live.file.id!==result.fileId)throw new Error('The year move completed, but the live file ID could not be rechecked. Reconnect before saving.');
  window.__renoweetCanonical=RenoweetDrive.clone(live.data);
  status('Year organized ✓',`${state.year} • same live file ID • ${result.movedProofs} receipts moved • checksum verified`);
  alert(`${state.year} is organized. The live file ID and checksum are unchanged. ${result.movedProofs} receipt files are now under the year folder. The Recovery snapshot and old folders remain for review.`);
 }catch(e){console.error(e);alert(`Year organization stopped: ${e.message}\n\nAny completed moves retain their file IDs. Reconnect and rerun Organize year to resume safely.`)}
}
function newBackupHTML(){return `<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(250px,1fr))"><div class="card"><h3>Live yearly database</h3><p class="small">One validated JSON database per year in your own Google Drive. Every load checks its integrity checksum before Renoweet accepts it.</p><div class="actions"><button class="btn primary" onclick="renoweetDriveOS.connect()">Connect Google Drive</button><button class="btn" onclick="renoweetDriveOS.refresh()">Refresh</button></div></div><div class="card"><h3>Import old quarterly XLSX</h3><p class="small">Choose the year and quarter first, preview invoices, projects and expenses, then import only missing records. Deterministic IDs and invoice checks prevent repeat imports.</p><button class="btn primary" onclick="location.href='Renoweet-Legacy-XLSX-Import-v4.4.html'">Open legacy importer</button></div><div class="card"><h3>Install verified database recovery</h3><p class="small">Install a merged yearly JSON only when its checksum and exact source revision still match Drive. Renoweet creates a Recovery snapshot first and stops if newer work exists.</p><input id="databaseRecoveryInput" type="file" accept=".json,application/json" hidden onchange="renoweetDriveOS.installRecovery(this.files?.[0]);this.value=''"><button class="btn primary" onclick="document.getElementById('databaseRecoveryInput').click()">Install recovery JSON</button></div><div class="card"><h3>Complete offline backup</h3><p class="small">One ZIP contains the exact JSON database, reconstructable XLSX and manifest. Store it anywhere you control.</p><div class="actions"><button class="btn primary" onclick="renoweetDriveOS.completeBackup()">Download complete ZIP</button><button class="btn" onclick="renoweetDriveOS.exportJSON()">JSON only</button><button class="btn" onclick="renoweetDriveOS.exportXLSX()">XLSX only</button></div></div><div class="card"><h3>Recovery & multi-device safety</h3><p class="small">Renoweet keeps local IndexedDB protection plus rotating Drive recovery snapshots. Saves check section revisions and verify the uploaded checksum before success.</p><button class="btn" onclick="renoweetDriveOS.save(true)">Save now</button></div><div class="card"><h3>Advanced: restore one project</h3><p class="small">Use a Renoweet single-project recovery JSON when one specific project was lost. The original Project ID is preserved and duplicates are blocked.</p><input id="projectRecoveryInput" type="file" accept=".json,application/json" hidden onchange="restoreProjectRecoveryFile(this.files?.[0]);this.value=''"><button class="btn" onclick="document.getElementById('projectRecoveryInput').click()">Restore project file</button></div><div class="card"><h3>Organize year folder</h3><p class="small">Preview the live database and receipt folders, make a verified Recovery snapshot, then move the same Drive IDs into the selected year. Old folders remain for review.</p><button class="btn primary" onclick="renoweetDriveOS.organizeYear()">Organize year</button></div><div class="card"><h3>Year navigation</h3><p class="small">The current year is editable. Earlier yearly JSON databases open read-only so you can safely browse old projects and customers.</p><div class="actions"><select id="driveYearSelectDatabase" class="btn" onchange="renoweetDriveOS.selectYear(this.value)"><option>${state.year}</option></select></div></div><div class="card"><h3>Data ownership</h3><p class="small">Stable internal IDs are preserved for projects, customers and financial records. The manifest points all Renoweet apps to the current year and archives.</p><button class="btn" onclick="renoweetDriveOS.settings()">Drive settings</button></div></div>`}
...
window.renoweetDriveOS={connect:connectDrive,save:saveDrive,refresh:refreshDrive,exportJSON:exportJson,exportXLSX:exportXlsx,completeBackup,installRecovery:installRecoveryDatabase,settings:driveSettings,selectYear,organizeYear:organizeDriveYear,state};
